...
// callClaude variant accepting pre-built messages array (for vision/media)
...
class DB {
  ...
  _default() {
    return {
      users: [{
        id: this._id(),
        name: 'Admin GerAI',
        email: 'pedro@ainoz.com.br',
        password: this._hashSync('Rj94kx30@geraiainoz'),
        role: 'admin',
        plan: 'elite',
        quota_limit: 99999,
        quota_used: 0,
  ...
  deleteUser(id) {
    const idx = this.data.users.findIndex(u => u.id === id);
    if (idx === -1) throw new Error('Usuário não encontrado');
    if (this.data.users[idx].email === 'pedro@ainoz.com.br') throw new Error('Admin master não pode ser excluído');
...
route('POST', '/api/user/generate-content', async (req, res) => {
  ...
  try {
    ...
    // Build message — text + optional media files
    let messages;
    if (mediaFiles && mediaFiles.length) {
      const content = [];
      mediaFiles.slice(0, 3).forEach(mf => {
        if (mf.isVideo) {
          // Videos: include as text description (Anthropic API doesn't support video natively)
          content.push({ type: 'text', text: `[MÍDIA ANEXADA: vídeo "${mf.name}" — analise o contexto descrito pelo usuário considerando que este é um vídeo de referência]` });
        } else {
...
MISSÃO: Criar arte gráfica para redes sociais — posts, carrosseis, stories. NÃO fotos de pessoas. Arte vetorial profissional.
...
${memoria ? '- Preferências de design: ' + memoria : ''}

SISTEMA DE DESIGN (OBRIGATÓRIO):
1. Background: rect 1080x1080 com ${cor2} (ou gradiente escuro baseado nele)
2. Elemento geométrico de estrutura: grid, linhas, formas abstratas em ${cor1} com opacidade 0.1-0.3
3. Bloco/shape de destaque em ${cor1}: ocupa 30-40% do espaço
4. Tipografia PRINCIPAL: texto dominante 80-140px, Bold, branco ou ${cor1}
5. Tipografia de suporte: 28-48px, cinza claro
6. Elementos decorativos: pontos, linhas finas, ícones geométricos
7. Hierarquia 60/30/10: fundo (60%), shape de cor (30%), destaque laranja/cor (10%)

FORMATOS:
- Post único (1080x1080): composição quadrada, elemento central dominante
- Story (1080x1920): adapte com viewBox="0 0 1080 1920", composição vertical
- Carrossel: slide único limpo, consistente com os outros slides
- Thumbnail YT: texto grande, contraste máximo, impacto visual imediato
- Banner: horizontal, texto à esquerda, visual à direita

${isInclude ? 'ATENÇÃO: O usuário quer INCLUIR a imagem de referência no design. Crie um espaço/moldura no SVG para a foto, use <image> tag com href vazio mas com espaço reservado indicado visualmente.' : 'ATENÇÃO: Use a imagem de referência apenas como inspiração de ESTILO (cores, layout, tipografia). NÃO reproduza o conteúdo da imagem.'}

QUALIDADE: O resultado deve parecer feito por um designer profissional de agência tier-1. Limpo, sofisticado, com hierarquia visual clara.`;

  const userMsg = `CONCEITO/CONTEÚDO: "${prompt}"
